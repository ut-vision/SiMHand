...
class F_DB(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # 2024.03.13  Debug Delete:
        '''
        # ----------------------- visualization ----------------------------- #
        import time
        # self.logger.debug(f'sample["joints3D"] value is {sample["joints3D"]}')
        # self.logger.debug(f'joints25D value is {joints25D}')
        from src.keypoints_vis.vis import draw_2d_skeleton
        from src.data_loader.freihands.utils.utils import get_params_from_camera_param
        from src.data_loader.freihands.utils.transforms import cam2pixel
        _, _, focal, princpt = get_params_from_camera_param(camera_param)
        # 需要注意，可视化的手指关节点坐标需要是mano格式的：
        joints3D = self.joints.ait_to_freihand(joints3D)
        # 需要注意，可视化的手指关节点坐标需要是mano格式的：
        joint_cam = joints3D
