...
class DefaultSampleAugmenter:
    def __init__(self, augmentation_flags: edict, augmentation_params: edict):
        ...
        """
        # Augmetation flags.
        self.set_augmentaion_flags_default(augmentation_flags)
    ...
    def transform_sample(
        ...
        image_original_shape = str(image_.shape)
    ...
    def get_crop_size(
        ...
        center_y, center_x = (
            int(torch.mean(joints[:, 1])),  # 1 = y
            int(torch.mean(joints[:, 0])),  # 0 = x
        )
