...
class BaseModel(LightningModule):
    ...
    def __init__(self, config: edict, logger_debug: None, mode: str):
        super().__init__()
        self.logger_debug = logger_debug    # Note from author: You can use it for debugging purposes
        self.mode = mode
    ...
    def configure_optimizers(self) -> Tuple[list, list]:
        ...
        optimizer = torch.optim.Adam(
            parameters,
            lr=self.config.lr
            * math.sqrt(1024 * self.config.num_of_mini_batch),
        )
        print(f"The pretrained models opt_weight_decay is set as: {self.config.opt_weight_decay}")
        print(f"The pretrained models learning rate is set as: {self.config.lr * math.sqrt(1024 * self.config.num_of_mini_batch)}")
