...
from src.models.utils import (
    ...
    apply_pca,
    get_weights_linear_with_pca,
    get_weights_nonlinear_with_pca,
)
from torch import Tensor
from torch.nn import functional as F


class HandCLR_W(SimCLR):
    """
    Implementation Data: 2024.04.24
    ...
    def get_adaptive_weights(
        ...
        if weight_type == 'linear':
            if self.config.use_pca != True:
                pos_weights, neg_weights = get_weights_linear(joints1, joints2, diff_type)
            else:
                joints1_pca = apply_pca(joints1, target_dim=14)
                joints2_pca = apply_pca(joints2, target_dim=14)
                pos_weights, neg_weights = get_weights_linear_with_pca(joints1_pca, joints2_pca, diff_type)
            ...
            if self.config.use_pca != True:
                ...
                pos_weights, neg_weights = get_weights_nonlinear_with_pca(apply_pca(joints1, target_dim=14), apply_pca(joints2, target_dim=14), self.config.non_linear_lambda_pos, self.config.non_linear_lambda_neg, diff_type)
