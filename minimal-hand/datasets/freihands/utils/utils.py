...
from .types import CAMERA_PARAM, JOINTS_3D, JOINTS_25D, SCALE
...
def get_params_from_camera_param(camera_param):
    # 假设 camera_param 是相机内参矩阵
    K, R, t = camera_param, np.eye(3), np.zeros((3, 1))  # 这里的 R 和 t 可以根据实际情况设置
    campos, camrot = t, R
    focal = [K[0, 0], K[1, 1]]
    princpt = [K[0, 2], K[1, 2]]
