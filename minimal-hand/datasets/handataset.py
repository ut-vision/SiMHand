...
class HandDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if 'cmu' in subset_name:
            ...
            logger.info(self.hand143_panopticdb)
            ...
            logger.info(colored(info, 'yellow', attrs=['bold']))
        ...
        # ------------------------- Realize by Ruicong: ah initalization ---------------------------- #
        if 'ah-exo' in subset_name or 'ah-ego' in subset_name:
        ...
        # ------------------------- Realize by Nie Lin:  fh initalization ---------------------------- #
        if 'fh' in subset_name:
            from.freihand import FreihandsDataset
            self.fh = FreihandsDataset(
                transforms.Compose([
                    transforms.ToTensor(),
                    transforms.ToPILImage(),
                    ]),
            ...
            logger.info(self.fh)
            # print(self.fh)
            self.datasets.append(self.fh)
        # ------------------------- Realize by Nie Lin:  fh initalization ---------------------------- #

        # ------------------------- Realize by Ohkawa:  dy initalization ---------------------------- #
        if 'dy' in subset_name:
            self.dy = DexYCBDataset(
                data_root=os.path.join(data_root, 'DexYCB'),
                data_split=data_split,
                hand_side=hand_side,
                njoints=njoints,
                # is_debug=is_debug,
                is_debug=False,  # is_debug = True/False
                #cache_pred_gen=cache_pred_gen,
                # setup=setup,
                setup='s1',
            )
            print(self.dy)
            self.datasets.append(self.dy)
        # ------------------------- Realize by Ohkawa:  dy initalization ---------------------------- #

        # ------------------------- Realize by Nie Lin:  fh initalization ---------------------------- #
        if 'ego4d' in subset_name:
            from.ego4d_hands import Ego4DHandsDataset
            self.ego4d_hands = Ego4DHandsDataset(
                transforms.Compose([
                    transforms.ToTensor(),
                    transforms.ToPILImage(),
                    ]),
            ...
            logger.info(self.ego4d_hands)
            # print(self.fh)
            self.datasets.append(self.ego4d_hands)
        # ------------------------- Realize by Nie Lin:  fh initalization ---------------------------- #
