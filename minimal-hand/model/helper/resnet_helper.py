...
# -- Modify 39: Pass the logger to record training process ---------------------------- #
    def __init__(self, logger, block, layers, num_classes=1000, zero_init_residual=False,
# -- Modify 39: Pass the logger to record training process ---------------------------- #    
        ...
        # -- Modify 40: Pass the logger to record training process ---------------------------- #
        self.logger = logger
        # -- Modify 40: Pass the logger to record training process ---------------------------- #
    ...
    def _forward_impl(self, x):
        ...
        # the shape of x is ([batch size, 3, 128, 128])
        x = self.conv1(x)   # after self.conv1, the shape of x is torch.Size([128, 64, 64, 64])
        x = self.bn1(x)     # after self.bn1, the shape of x is torch.Size([128, 64, 64, 64])
        x = self.relu(x)    # after self.relu, the shape of x is torch.Size([128, 64, 64, 64])
        x = self.block1(x)  # after self.block1. the shape of x is torch.Size([128, 256, 64, 64])
        x = self.pool(x)    # after self.pool, the shape of x is torch.Size([128, 256, 32, 32])
        x = self.block2(x)  # after self.block2, the shape of x is torch.Size([128, 512, 32, 32])
        x = self.block3(x)  # after self.block3, the shape of x is torch.Size([128, 1024, 32, 32])
        x = self.squeeze(x) # after self squeeze, the shape of x is torch.Size([128, 256, 32, 32])
        return x

    def forward(self, x):
        return self._forward_impl(x)

# -- Modify 38: Pass the logger to record training process ---------------------------- #
def _resnet(arch, logger, block, layers, pretrained, progress, **kwargs):
    model = ResNet(logger, block, layers, **kwargs)
    return model
# -- Modify 38: Pass the logger to record training process ---------------------------- #
...
# -- Modify 37: Pass the logger to record training process ---------------------------- #
def resnet50(logger = None, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', logger, Bottleneck, [2, 4, 6], pretrained, progress,
                   **kwargs)
# -- Modify 37: Pass the logger to record training process ---------------------------- #
...
def resnet152(logger = None, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet152', logger, Bottleneck, [3, 8, 36, 3], pretrained, progress,
                   **kwargs)
